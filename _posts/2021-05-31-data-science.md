...
1. **Data Exploration and Preparation**: These would involve the following:
    - _Exploration_: Open-ended "detective" work to explore data and expose unexpected features
    - _Preparation_: Identifying anomalies, artifacts, missing data... all activites involved in "cleaning" data.
2. **Data Representation and Transformation**: Implementing appropriate transformation to restructure data to reveal more insights. This is one key area in which how data-scientists' work will differ from that of statisticians. 
3. **Computing with Data**: Data-scientists would need to apply elements from "traditional" computer science, but applied to large volumes of data. These could be multiple tasks involving data analysis and processing, computational optimization, cloud computing, developing workflows, repeatable artifacts and so on. 
4. **Data Modeling**: Leo Breiman suggests there are two modelling "cultures":
    - _Generative Modelling_ (Traditional Statistics): A single statistical model is proposed that could generate the data. Methods to infer properties of the underlying model are also proposed
...
In the next few posts, I will hit upon a few of these elements, perhaps with some examples to boot. Stay tuned.